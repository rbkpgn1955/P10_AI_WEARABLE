...
#define STORAGE_DEVICE_SD_CARD              (1)
...
static TaskHandle_t pdm_task_handle;
static cyhal_gpio_callback_data_t user_btn_callback_data, pdm_callback_data;
...
/*******************************************************************************
* Function Name: pdm_task
********************************************************************************/
void write_audio_frame(lfs_t *lfs, struct lfs_config *lfs_cfg)
{
	if(STORAGE_DEVICE_SD_CARD){
		lfs_file_t file;
		//check_status("Creating SD card block device failed", result);
		...
		/* Read the current boot count. */
		lfs_file_open(lfs, &file, "frame_count", LFS_O_RDWR | LFS_O_CREAT);

		/* write frame to file */
		lfs_file_write(lfs, &file, &audio_frame[0], sizeof(audio_frame[0]));
...
int main(void)
{
    ...
    xTaskCreate(pdm_task, "PDM Task", 512U, NULL, (configMAX_PRIORITIES - 1), &pdm_task_handle);
